...
const AINotebookGenerator = ({
  ...
  const handleDownload = () => {
    ...
    if (generatedNotebook && generatedNotebook.downloadUrl) {
      // Open the S3 download URL in a new tab (browser will handle download)
      window.open(generatedNotebook.downloadUrl, '_blank');
    } else {
      alert('Notebook download URL not found. Please try again.');
    }
  };
    ...
    <div className="text-center py-8">
      ...
      <div className="flex justify-center space-x-3">
        ...
        <button
          onClick={() => {
            if (generatedNotebook && generatedNotebook.downloadUrl) {
              window.open(`/jupyterlite/index.html?notebookUrl=${encodeURIComponent(generatedNotebook.downloadUrl)}`, '_blank');
            } else {
              alert('Notebook download URL not found. Please try again.');
            }
          }}
