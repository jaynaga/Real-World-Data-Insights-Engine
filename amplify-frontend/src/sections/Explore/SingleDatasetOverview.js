...
import { FiDownload, FiShare2, FiBarChart2, FiMap, FiCalendar, FiUsers, FiTag, FiArrowLeft, FiZap, FiDatabase, FiFileText, FiEye } from 'react-icons/fi';
...
export default function SingleDatasetOverview({ datasets = [], loading = false }) {
  ...
  // Find the dataset by ID from the datasets array
  const dataset = useMemo(() => {
    const foundDataset = datasets.find(d => String(d.id) === String(datasetId));
    return foundDataset;
  }, [datasets, datasetId]);
  ...
  useEffect(() => {
    if (dataset?.files && dataset.files.length > 0 && !selectedFile) {
  ...
  const handleFileSelect = (file) => {
    ...
    setCsvPreview(null); // Clear previous preview
  };
